...
// Connexion à la base de données avec une pool de connexions
...
pool.getConnection()
  .then(connection => {
    ...
    app.post('/api/login', async (req, res) => {
      ...
      try {
        const [rows] = await connection.execute('SELECT * FROM Admin WHERE User = ? AND Password = ?', [username, password]);
        if (rows.length > 0) {
          const user = rows[0];
        ...
        handleDBError(res, err, 'Erreur lors de la recherche de l\'utilisateur dans la base de données :');
      }
    });
    ...
    // Autres routes et middleware ici...
